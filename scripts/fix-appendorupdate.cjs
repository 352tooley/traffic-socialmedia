...
async function main() {
  ...
  for (const node of workflow.nodes) {
    if (node.name === 'Save Traffic Data') {
      console.log('Changing to appendOrUpdate operation...');
  ...
  if (updateRes.ok) {
    console.log('SUCCESS! Workflow now uses appendOrUpdate');
    console.log('This will update existing rows or append if not found');
  } else {
